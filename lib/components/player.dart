...
with HasGameReference<MyGame>, KeyboardHandler, CollisionCallbacks {
  ...
  FutureOr<void> onLoad() async{
    ...
    add(RectangleHitbox.relative(
      Vector2(0.6, 0.9), parentSize: size,
  ...
  void _handleScreenBounds(){
    ...
    /// prevent the player for going to the top and down with the jet  
    position.y = clampDouble(
      ...
      // prevent the player for going to the left and right with the jet 
  ...
  void _handleDestruction()async{
    animation = SpriteAnimation.spriteList(
      ...
      add(ColorEffect(
        ...
        add(MoveEffect.by(
          Vector2(0,200), EffectController(duration: 3.0)),
          );
  ...
  void _createRandomExplosion(){
    final Vector2 explosionPosition = Vector2(
      ...
      position.y - size.x/2 +_random.nextDouble() * size.y,);

      final ExplosionType explosionType = 
      _random.nextBool() ? ExplosionType.smoke : ExplosionType.fire;

      final Explosion explosion = Explosion(position: 
      explosionPosition, explosionSize: size.x * 0.7,
